...
public class DemoTableCopyProcessor {
			...
			if (dmuHistoryDetailList != null && !dmuHistoryDetailList.isEmpty()) {
				dmuHistoryDetailList.parallelStream().forEach(historyDetailEntity -> {
					...
					if (StringUtils.isBlank(hdfspath)) {
						historyDetailEntity.setStatus(Constants.FAILED);
	...
	private String retrieveHDFSPath(DMUHistoryDetail dmuHistoryDetail) {
		log.info(" called=> ScriptGenerationService ::  proceedScriptGenerationForRequest :: invokeHDFSService ");
		try {
	...
	private void processTableCopyHelper(DMUHistoryDetail dmuHistoryDetail, String hdfsPath, ConnectionDto connectionDto,
			String requestNo) {
		log.info(
	...
	public void populateTGTOtherProperties(ConnectionDto connectionDto) {
		...
		if (tgtOtherProp.isPresent()) {
			...
			connectionDto.setTgtOtherPropDto(TGTOtherPropDto.builder().build());
		}
	}

	private void populateTGTFormatProperties(ConnectionDto connectionDto) {
		try {
			Optional<TGTFormatProp> tgtFormatPropOpt = tgtFormatPropRepository.findById(1L);
			if (tgtFormatPropOpt.isPresent()) {
				TGTFormatProp tgtFormatPropEntity = tgtFormatPropOpt.get();
				connectionDto.setTgtFormatPropDto(TGTFormatPropDto.builder().srNo(tgtFormatPropEntity.getSrNo())
