...
public interface HistoryDetailRepository extends JpaRepository<DMUHistoryDetail, DMUHIstoryDetailPK> {
	...
	@Query("SELECT detail from DMUHistoryDetail detail where detail.dmuHIstoryDetailPK.requestNo = :requestNo AND detail.status = :status ORDER BY detail.dmuHIstoryDetailPK.requestNo ASC")
