 ...
 * 
 * //@Configuration public class DMUHivePersistenceConfig {
 * 
 * @Bean(name = "hiveJdbcDataSource")
 * 
 * @ConfigurationProperties(prefix = "hs2.datasource") public DataSource
 * dataSource() { return DataSourceBuilder.create().build(); }
 * 
 * @Bean(name = "hiveJdbcTemplate") public JdbcTemplate
 * hiveJdbcTemplate(@Qualifier("hiveJdbcDataSource") DataSource dataSource) {
 * return new JdbcTemplate(dataSource); }
 * 
 * 
 * @PostConstruct public void init() { Connection con = null; try {
 * Class.forName("com.cloudera.hive.jdbc41.HS2Driver"); } catch
 ...
 * stmt.executeQuery("SHOW DATABASES"); while (rs.next()) {
 * System.out.println(rs.getString(1)); }
 * 
 * con.close(); } catch (SQLException e) { // TODO Auto-generated catch block
 * e.printStackTrace(); } System.out.println("connected");
 * 
